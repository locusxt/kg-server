 ...
 * @Last Modified time: 2018-01-02 12:26:03
 */
...
async function(uid, pid, eid){
    ...
    var cypher = "START u=node({uid}), p=node({pid}), e=node({eid}) " +
                "MATCH (u)-[:refer]->(e) " +
                "MATCH (e)-[:in]->(p) " +
                "MATCH (e)<-[:hasTarget]-(:RoleInst)<-[:hasRole]-(tag:RelInst {tag:'instance_of', tagid:-2})-[:hasRole]->(:RoleInst)-[:hasTarget]->(parent:Entity:Model)" +
                "MATCH (u)-[:refer]->(tag) " +
                "RETURN parent.id";
    return new Promise((resolve, reject) => {
        db.query(cypher, {uid : user.id, pid : project.id, eid:eid}, (err, res) => {
